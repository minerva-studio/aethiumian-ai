    ...
    public sealed class FlyNavigationPlanner : NavigationPlanner<FlyNavigationParameters>
    {
        ...
        public override NavigationPlanResult Plan(AABB body, NavigationGoalRequest goal, FlyNavigationParameters parameters, CancellationToken cancellationToken = default, NavigationPlanningDiagnostics diagnostics = null)
        {
            ...
            if (start.Equals(resolvedGoal))
                return NavigationPlanResult.ResultProduced(NavigationRoute.Empty(start, NavigationRouteCoordinateFrame.BodyCenter, true));

            bool directClear = IsCenteredSegmentClear(World, start, resolvedGoal, bodySize, cancellationToken);
            cancellationToken.ThrowIfCancellationRequested();
            if (directClear)
                return NavigationPlanResult.ResultProduced(NavigationRoute.CreateSingleSegment(new FlyRouteSegment(start, resolvedGoal), true));
        ...
        public override NavigationPlanResult PlanSingleStep(AABB body, NavigationGoalRequest goal, FlyNavigationParameters parameters, CancellationToken cancellationToken = default)
        {
            ...
            NavigationRoute route = NavigationRoute.CreateSingleSegment(new FlyRouteSegment(start, endpoint), World.IsGoalComplete(goal, CenteredBody(endpoint, bodySize)));
        ...
        private NavigationPlanResult PlanRetreat(Vector2 start, NavigationGoalRequest goal, FlyNavigationParameters parameters, Vector2 bodySize, CancellationToken cancellationToken, NavigationPlanningDiagnostics diagnostics)
        {
            ...
            if (parameters.RemainingApproachDistance is float startApproachLimit && startApproach > startApproachLimit + Tolerance)
                return NavigationPlanResult.NoResult;
            ...
            while (expanded < MaxExpandedNodes && TrySelectRetreatNext(open, labels, out int currentLabelId))
            {
                ...
                {
                    return NavigationPlanResult.ResultProduced(BuildRetreatRoute(start, World, labels, currentLabelId));
                }
                ...
                for (int i = 0; i < Directions.Length; i++)
                {
                    ...
                    if (!IsInsideWorld(World, nextPosition))
                        continue;
                    if (!IsFlyBodyClear(World, nextPosition, bodySize) || !IsFlyBodyPathClear(World, currentPosition, nextPosition, bodySize))
                        continue;
                    ...
                    if (parameters.RemainingApproachDistance is float approachLimit2 && candidateApproach > approachLimit2 + Tolerance)
                        continue;
                    RetreatSearchLabel candidate = new(next, currentLabelId, candidateCost, candidateApproach, World.GetGoalCompletionDistance(goal, CenteredBody(nextPosition, bodySize)));
                    if (!TryAddRetreatLabel(labels, labelIdsByCell, candidate, out int candidateId))
                        continue;
        ...
        private static NavigationRoute BuildRetreatRoute(Vector2 start, INavigationWorld snapshot, List<RetreatSearchLabel> labels, int labelId)
        {
            List<int> chain = new();
            ...
            return BuildFlyRoute(start, snapshot, cells, null);
        }

        private static NavigationRoute BuildFlyRoute(Vector2 start, INavigationWorld snapshot, IReadOnlyList<Vector2Int> cells, Vector2? finalEndpoint)
        {
            ...
            return NavigationRoute.Complete(segments);
        }
        ...
        private static bool TryAddRetreatLabel(List<RetreatSearchLabel> labels, Dictionary<Vector2Int, List<int>> labelIdsByCell, RetreatSearchLabel candidate, out int candidateId)
        {
            ...
            for (int index = 0; index < cellLabelIds.Count; index++)
            {
                ...
                if (existing.State == RetreatSearchLabelState.Discarded)
                    continue;
            ...
            for (int index = 0; index < cellLabelIds.Count; index++)
            {
                ...
                if (existing.State == RetreatSearchLabelState.Discarded)
                    continue;
                if (candidate.PathCost <= existing.PathCost + Tolerance && candidate.ApproachCost <= existing.ApproachCost + Tolerance)
                {
        ...
        private static bool TrySelectRetreatNext(NavigationMinHeap<HeapKey> open, List<RetreatSearchLabel> labels, out int labelId)
        {
            ...
            while (!open.IsEmpty)
            {
                ...
                if (queuedLabelId < 0 || queuedLabelId >= labels.Count)
                    continue;
                RetreatSearchLabel label = labels[queuedLabelId];
                if (label.State != RetreatSearchLabelState.Open)
                    continue;
                if (Mathf.Abs(queuedScore - label.RetreatScore) > Tolerance)
                    continue;
        ...
        private static bool TryFindRetreatConnectorCell(INavigationWorld snapshot, Vector2 position, Vector2 targetCenter, FlyNavigationParameters parameters, Vector2 bodySize, out Vector2Int connector)
        {
            ...
            for (int y = origin.y - 1; y <= origin.y + 1; y++)
            {
                for (int x = origin.x - 1; x <= origin.x + 1; x++)
                {
                    ...
                    if (parameters.RemainingApproachDistance is float connectorApproachLimit && approach > connectorApproachLimit + Tolerance) continue;
        ...
        /// <summary>
        /// Converts a Ground Range lower-center goal into the body center used by Fly.
        /// </summary>
        ...
        /// <summary>
        /// Converts one resolved lower-center candidate into Fly's center-anchor space.
        /// </summary>
        private static Vector2 GetGoalCenter(NavigationGoalRequest goal, Vector2 bodySize, Vector2 candidate)
            => goal.IsGroundWalk ? candidate + Vector2.up * (bodySize.y * 0.5f) : candidate;
        ...
        {
            if (parameters.RemainingApproachDistance is float remainingApproachDistance && (!NavigationNumeric.IsFinite(remainingApproachDistance) || remainingApproachDistance < 0f))
