...
>注意:所有的变量的类型与初始值都在行为树的实例生成前就已经生成,变量的类型不可以在运行的时候更改.
...
### NodeProgress（适用于ScriptCall节点和ScriptAction节点的参数）
用于控制一个节点的执行状态。在ScriptAction与ScriptCall中，被指定的方法如果具有该参数，则该方法可以通过控制NodeProgress来实现对树的控制
#### 方法
```c#
//暂停Behaviour Tree的执行
public void Pause();
...
#### 举例：
```c#
//example: 用ScriptCall来执行脚本中的Attack方法
public void Attack(NodeProgress progress){
	if(...){
		//使得ScriptCall节点返回false
		progress.End(false);
	}
	//使得ScriptCall节点返回true
...
public bool MethodName(); //仅ScriptCall
...
- 参数  
	...
	> 该方法的寻找方式较为特殊，参考章节 [NodeProgress](#nodeprogress适用于scriptcall节点和scriptaction节点)
  ...
  | byMethod      | 在指定方法返回内结束该Action （必须通过`NodeProgress.End()`） |
    > 如果指定的Action的结束方式为byMethod，则ScriptAction不可以接受一个没有参数`NodeProgress`的方法，因为这样该Action将无法结束
...
- 参数  
   ...
   - `ParentMode parentOfObject`: 新的GameObject的parent
   ...
   - `OffsetMode offsetMode`: 新的GameObject的位置
     |  OffsetMode  |                                               |
     | :----------: | :-------------------------------------------: |
...
- 参数  
	- `TreeNode condition`
		条件节点  
		当condition结果为true,行为树将回到该Service所绑定的节点
...
### CustomNodeDrawerBase [code](../Editor/AIEditor/CustomNodeDrawerBase.cs)
...
### AIEdtor [code](../Editor/AIEditor/AIEditor.cs)
AI Editor窗口的脚本
